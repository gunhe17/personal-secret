---
description: 기존 aggregate에 usecase 동작 하나(create/update/reveal/list/delete 등)를 추가할 때. Input(In)+Output(Out) + 함수(-> Output) + CLI 절차.
---
...
1. 파일 — `usecase/{aggregate}_{action}.py` (폴더 없이 평탄, `# input` / `# output` / `# usecase` / `# cli` 섹션)
2. Input/Output — `class Input(In)` (필드 없어도 정의, `by_factory` 가드 없음) + `class Output(Out): pass`. core(`core/usecase.py`)의 추상 `In`/`Out` 상속
3. 함수 — `@typecheck async def {action}(*, session, input: Input) -> Output`
...
5. 이벤트(쓰기면) — 마커 생성 + `EventRepository.emit(session=session, id=..., name=..., atomics=[...])`, `Output.new(data=..., event=[...])` 응답 ([reference/domain-event.md](../../../../../.claude/rules/api/domain-event.md))
...
- [ ] 응답이 `Output.new(data, event)`인가 — 맨몸 dict 금지, endpoint가 `.to_dict()` ([INV-8])
