...
루트: [api.md](../api.md) · entity: [entity.md](entity.md) · 예외: [exception.md](exception.md)
...
- **CRUD**: 생성 `add`/`add_many`, 조회 public `find_by_id`/`get_by_ids`/`exists_by_id`/`list_all`, 수정 `update`/`update_many`, 삭제 `remove_by_id`
...
유일성은 **base 사전검사 guard + 도메인 변환**의 2계층. domain repo가 `{action}_unique_by_{col}`로 노출한다.

- **base `_ensure_unique(*, session, entity, unique: list[str], exclude_id=None) -> None`**: `unique` 컬럼마다 `_count` 검사, 있으면 **infra `UniqueViolationError`**(`postgresql/exception.py`, 409) raise. update 자기 행은 `exclude_id`로 제외. `deleted_at IS NULL` 자동(soft-delete된 이름 재사용 허용, partial unique index와 일치)
- **domain 변환**: `try/except`로 `UniqueViolationError`(infra) → `AlreadyExistsError`(domain). base는 도메인 예외를 모른다
- 메서드명에 유일 컬럼 명시(`add_unique_by_name`/`update_unique_by_name`), 복합키는 `add_unique_by_name_and_kind` + `unique=["name", "kind"]`
