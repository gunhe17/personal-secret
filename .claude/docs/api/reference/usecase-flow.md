...
비즈니스 흐름 — deps + input → output. transaction 경계를 책임진다. `usecase/{aggregate}/{action}.py`에 산다.

루트: [api.md](../api.md) · repo: [repository.md](repository.md) · 이벤트 조정: [domain-event.md](domain-event.md)
...
- 클래스명 `Input` 고정 — 파일명(`{action}`)이 컨텍스트 제공
...
- `session = AsyncSession`(트랜잭션 경계), `input = Input`만 받음 — repo는 클래스로([repository.md](repository.md), **[INV-6]**), 인프라 싱글톤(`crypto`/`session_cache`)은 모듈 import로 직접
...
read-only(`reveal`, 쓰기 없음 → `# persist` 생략):

```python
@typecheck
async def reveal(*, session, input: Input) -> dict:
    # find
    secret = await SecretRepository.get_by_identifier(session=session, id=input.id)

    # reveal
    data = json.loads(
        session_cache.decrypt(data=secret.ciphertext.to_bytes()).decode("utf-8")
    )

    # return
    return {**secret.to_dict(), "data": data}
```
...
- `print(await {action}(...))`는 [conventions.md](../conventions.md) "nested call 펼침"대로 줄바꿈. `print(await {action}(` 병합형 금지
...
**transaction에 안 들어가는 것**(DB 밖 side-effect는 rollback 안 됨): in-memory cache(`session_cache.unlock(dek=...)`), 외부 호출. side-effect → DB 순서로 두면 DB 실패 시 outer state 일관.
