...
| 시그니처 | `(*, session, input: Input) -> Output`. repo는 클래스로 호출([INV-6]), 싱글톤은 모듈 import |
| 본문 라벨 | `# find → # {동작} → # persist → # return` 흐름 단계 마커 |
| Session = Transaction | 같은 session을 모든 repo 호출에 주입 = atomic. usecase 1개 = 1 트랜잭션 — [INV-5] |
| 응답 | `Output.new(data, event)` 반환, endpoint가 `to_dict()` — [INV-8] |
...
- `class Output(Out): pass` — 빈 상속. `Output.new(...)`가 이 로컬 타입을 돌려주고 `to_dict()`는 `Out`에서 상속. 반환 어노테이션 `-> Output`은 로컬 타입
...
```python
@typecheck
async def {action}(*, session, input: Input) -> Output:
```
...
```python
@typecheck
async def update(*, session, input: Input) -> Output:
    # find
...
### 응답 shape — `Output.new(data, event)` (INV-8)

usecase는 **언제나 `Output.new(data=..., event=...)`를 반환**한다(로컬 `Output`, `Out` 상속). `to_dict()`가 `{"data", "event"}` 두 키를 항상 내므로 read/write 무관하게 shape 동일. endpoint는 `result.to_dict()`로만 직렬화(재가공·래핑 금지 — [endpoint.md](endpoint.md)).
`event`는 기본값 없음 — read/write 모두 명시한다.
- write: `Output.new(data=..., event=[...])` — 저장된 이벤트 echo
- read: `Output.new(data=..., event=None)` — `None` 명시. 이벤트는 발생·기록했으나 응답엔 echo 안 함(`[]` 아닌 `None`)
- list: `Output.new(data=[...], event=None)` — `data`에 맨몸 `list` OK(`Output`이 `{"data": [...]}`로 감쌈), usecase가 dict로 직접 래핑 금지
...
- 응답을 맨몸 dict로 → `Output.new(data=..., event=...)` 반환 ([INV-8])
