...
- 규칙은 일반화 — 본문엔 일반 규칙만. 특정 구현(클래스·함수명·배선)은 `# good:`/`# bad:` 예시로 강등한다. 구현을 규칙 문장에 박으면 문서가 그 프로젝트·시점에 묶여 드리프트한다 — 규칙은 살리고 사례는 예시로.
...
- 구현 세부를 규칙 문장에 인라인 → 규칙은 일반어로, 사례는 예시 블록으로 강등
